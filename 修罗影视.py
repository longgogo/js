...
_LINES_CACHE = {}  # pid -> (expire_epoch, lines_data)
...
class Spider(Spider):
    ...
    def playerContent(self, flag, id, vipFlags):
        # 预取并缓存播放数据：localProxy 命中缓存可秒回，避免播放器等本地代理超时
        try:
            self._resolve_m3u8(id)
        except Exception:
            pass
    ...
    def _http_get_bytes(self, url, headers=None, jar=None, range_end=None, timeout=12):
        h = {"User-Agent": CH_UA}
        if headers:
            h.update(headers)
        ...
        return r.read(range_end + 1) if range_end is not None else r.read()
    ...
    def _get_lines(self, pid, jar):
        ...
        if d.get("code") != 0:
            return None
        _LINES_CACHE[str(pid)] = (time.time() + 60, d.get("data") or {})
    ...
    def _prefixes_for(self, blob_url):
        """分片绝对前缀候选：默认 vod.xl01.me（blob 带 us 标记时加 /hls/），站点域名作兜底。"""
        pre = "https://vod.xl01.me/hls/" if ("2014-us" in blob_url or "ac5634-us" in blob_url) \
            else "https://vod.xl01.me/"
        return [pre, self.host + "/"]
    ...
    def _validate_m3u8(self, text, prefix):
        """线路验真：Range 只取首 376 字节验 TS 同步字节 0x47（部分线路分片是 PNG 伪装废料）。
        头部分片可能是广告位，长度足够时再抽查中段一个。"""
        try:
            ...
            picks = [segs[0]]
            if len(segs) > 20:
                picks.append(segs[len(segs) // 2])
            for s in picks:
                u = s if s.startswith("http") else prefix + s.lstrip("/")
                raw = self._http_get_bytes(u, range_end=375, timeout=10)
                if not raw or raw[0] != 0x47:
                    return False
            return True
        except Exception:
            return False
    ...
    def _fetch_m3u8_text(self, pid):
        ...
        for u in cands:
            ...
            try:
                raw = self._http_get_bytes(u, {"Referer": self.host + "/"}, jar=jar, timeout=15)
                txt = self._decode_blob(raw)
                if not txt:
                    continue
                prefixes = self._prefixes_for(u)
                for pre in prefixes:
                    if self._validate_m3u8(txt, pre):
                        return self._rewrite_m3u8(txt, pre)
                if first_txt is None:
                    first_txt = self._rewrite_m3u8(txt, prefixes[0])
            except Exception:
                continue
        # 兜底：/god?type=1（本机出口实测被上游 CDN 拒，部分网络可用）
        try:
            now = str(int(time.time() * 1000))
            ...
            if gu:
                raw = self._http_get_bytes(gu, {"Referer": self.host + "/"}, jar=jar, timeout=15)
                txt = self._decode_blob(raw)
                if txt:
                    prefixes = self._prefixes_for(gu)
                    for pre in prefixes:
                        if self._validate_m3u8(txt, pre):
                            return self._rewrite_m3u8(txt, pre)
                    first_txt = first_txt or self._rewrite_m3u8(txt, prefixes[0])
        except Exception:
            pass
