...
说明 :
  ...
  4. 若主域名被 Cloudflare 拦截，会自动切换到备用镜像域名。
================================================
"""
...
# CDN 域名轮换名单（与站点 xlplayer.js 内逻辑一致）
CDN_LIST_I = [
    'f19-', 'f77-', 'lf16-capcut.faceueditorv.com', 'lf16-secsdk.bitssec.com',
    'lf16-data.bytepluscdn.com', 'lf16-fdm.fdmimgs.com',
    'lf16-cc-everphoto.faceulv.com', 'lf16-capcut.faceulv.com',
...
class Spider(_BaseSpider):
    ...
    # ---------------- 工具 ----------------
    def _fix_cdn(self, u):
        """还原 CDN 真实可用域名（对应 xlplayer.js 的域名重写逻辑）"""
        i = u.find('tos-alisg-i-0000')
        if i > 0:
            if any(h in u for h in CDN_LIST_I):
                return 'https://lf16-anyweb.anyweb.space/obj/' + u[i:]
            return 'https://p16-hera-sign-va.ibyteimg.com/obj/' + u[i:]
        j = u.find('tos-alisg-v-0000')
        if j > 0:
            if any(h in u for h in CDN_LIST_V):
                return 'http://lf77-geckocdn.bytegecko-i18n.com/obj/' + u[j:]
            return 'https://p16-hera-sign-va.ibyteimg.com/obj/' + u[j:]
        k = u.find('tos-maliva-o-0000-us')
        if k > 0:
            if any(h in u for h in CDN_LIST_O):
                return 'https://lf16-gcdn.maththuag.com/obj/' + u[k:]
            return 'https://p16-hera-sign-va.ibyteimg.com/obj/' + u[k:]
        return u
